...
class PrincipledBakerOperator(bpy.types.Operator):
    ...
    def new_image(self, image_file_name):
        image = bpy.data.images.new(
                    name=image_file_name, 
                    alpha=self.settings.use_alpha, 
    ...
    def execute(self, context):        
                ...
                if not 'p_baker_material' in mat.keys():
                    ...
                    if material_output == None:
                        ...
                        if material_output.inputs['Surface'].is_linked:
                            material_data[mat]['socket_to_surface'] = material_output.inputs['Surface'].links[0].from_socket
                        if material_output.inputs['Displacement'].is_linked:
                            material_data[mat]['socket_to_displacement'] = material_output.inputs['Displacement'].links[0].from_socket
        ...
        for obj in selected_objects:
            ...
            for input_name in self.joblist:
                ...
                alpha_channel = 0.0 if self.settings.use_alpha else 1.0
                if input_name == 'Bump':
                        ...
                        if not 'p_baker_material' in mat.keys():
                            ...
                            if input_name == 'Displacement':
                                ...
                                if not material_output.inputs['Surface'].is_linked:
                                    self.report({'WARNING'}, 'Surface Input missing in Material Output in "{0}"'.format(mat.name))
                                    return {'CANCELLED'}
                                else:
                                    socket_to_surface = material_output.inputs['Surface'].links[0].from_socket
                            ...
                            if input_name == 'Alpha':
                                ...
                                self.prepare_for_bake_factor(mat, socket_to_surface, socket_to_pb_diffuse_node, node_type)
                            elif input_name == 'Displacement':
                                if material_output.inputs['Displacement'].is_linked:
                                    self.prepare_for_bake(mat, socket_to_displacement, socket_to_pb_diffuse_node, input_name)
                            else:
                ...
                for mat in material_data:
                    if 'p_baker_material' not in mat.keys():
                        ...
                        if not material_data[mat]['socket_to_surface'] == None:
                            mat.node_tree.links.new(material_data[mat]['socket_to_surface'], material_output.inputs['Surface'])
